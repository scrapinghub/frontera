...
class TestFronteraScheduler:
    ...
    def test_redirect_enabled_enqueue_requests(self):
        ...
        assert {request.url for request in fs._pending_requests} == {rr1.url, rr2.url, rr3.url}
    ...
    def test_next_request_overused_keys_info(self):
        ...
        assert set(fs.frontier.manager.get_next_requests_kwargs[0]['overused_keys']) == {'2.1.3', '4.1.3'}
    ...
    def test_process_spider_output(self):
        ...
        assert {r.url for r in fs.frontier.manager.links} == {r.url for r in requests}

        assert isinstance(fs.frontier.manager.responses[0], FResponse)
        assert fs.frontier.manager.responses[0].url == resp.url
        assert {request.url for request in fs.frontier.manager.links} == {r1.url, r2.url, r3.url}

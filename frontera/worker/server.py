...
from frontera.utils.async_ import listen_tcp
    ...
    def render_POST(self, txrequest):
        ...
        try:
            try:
                ...
                raise JsonRpcError(
                    500, f"Error processing request: {''.join(trace_lines)}"
                ) from err
...
class JsonRpcService(server.Site):
    ...
    def start_listening(self):
        ...
        logger.info(f"Web service listening on {address.host}:{address.port}")
...
class WorkerJsonRpcService(JsonRpcService):
    ...
    def start_listening(self):
        ...
        address = self.port.getHost()
        self.worker.set_process_info(f"{address.host}:{address.port}")

...
class FrontierManagerWrapper:
    ...
    def start(self):
        if not hasattr(self, "request_converter"):
            raise NotImplementedError(
                "Request converter should be instantiated in subclass"
            )
        if not hasattr(self, "response_converter"):
            raise NotImplementedError(
                "Response converter should be instantiated in subclass"
            )
        assert isinstance(self.request_converter, BaseRequestConverter), (
            "request_converter must be instance of BaseRequestConverter"
        )
        assert isinstance(self.response_converter, BaseResponseConverter), (
            "response_converter must be instance of BaseResponseConverter"
        )
        self.manager.start()
    ...
    def links_extracted(self, request, links):
        ...
        self.manager.links_extracted(
            request=self.request_converter.to_frontier(request), links=frontier_links
        )

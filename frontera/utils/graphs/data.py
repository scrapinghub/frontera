def create_test_site(
    prefix, max_depth, n_links_per_page, self_link=False, site=None, depth=0
):
    if not site:
        site = []
        prefix += str(1)
...
# Sites
# -----------------------------------------------------
SITE_A = CrawlSiteData(name="A", description="", pages=create_test_site("A", 4, 2))

SITE_B = CrawlSiteData(name="B", description="", pages=create_test_site("B", 4, 2))

SITE_C = CrawlSiteData(
    name="C", description="", pages=create_test_site("C", 5, 2, self_link=True)
)

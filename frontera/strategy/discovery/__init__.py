...
def is_domain_to_ignore(domain, max_pages):
    return (not is_accessible_domain(domain) or 'banned' in domain or domain.setdefault('queued_pages', 0) >= max_pages)
...
class Discovery(BaseCrawlingStrategy):
    ...
    def _get_domain_after_redirects(self, request):
        ...
        if redirect_urls and (b'robots' in request.meta or b'sitemap' in request.meta or b'home' in request.meta):

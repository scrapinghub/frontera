...
def _prepare_request_message(request):
    def serialize(obj):
        ...
        if isinstance(obj, tuple):
            return tuple(serialize(list(obj)))
        if hasattr(obj, "__dict__"):
            return serialize(obj.__dict__)
        logger.warning(f"unable to serialize object: {obj}")
        return None
...
def _prepare_response_message(response, send_body):
    return [
        ...
        response.meta,
        response.body if send_body else None,
    ]


class Encoder(BaseEncoder):
    def __init__(self, request_model, *a, **kw):
        self.send_body = bool(kw.get("send_body"))

    def encode_add_seeds(self, seeds):
        return packb(
            [b"as", [_prepare_request_message(seed) for seed in seeds]],
            use_bin_type=True,
        )

    def encode_page_crawled(self, response):
        return packb(
            [b"pc", _prepare_response_message(response, self.send_body)],
            use_bin_type=True,
        )
    ...
    def encode_links_extracted(self, request, links):
        return packb(
            [
                b"le",
                _prepare_request_message(request),
                [_prepare_request_message(link) for link in links],
            ],
            use_bin_type=True,
        )
    ...
    def encode_request_error(self, request, error):
        return packb(
            [b"re", _prepare_request_message(request), str(error)], use_bin_type=True
        )
    ...
    def encode_update_score(self, request, score, schedule):
        return packb(
            [b"us", _prepare_request_message(request), score, schedule],
            use_bin_type=True,
        )
...
class Decoder(BaseDecoder):
    ...
    def _response_from_object(self, obj):
        ...
        return self._response_model(
            ...
            status_code=obj[1],
            body=obj[3],
            request=self._request_model(url=url, meta=obj[2]),
        )

...
class Encoder(BaseEncoder):
    ...
    def encode_add_seeds(self, seeds):
        return packb(['as', [_prepare_request_message(seed) for seed in seeds]])

    def encode_page_crawled(self, response, links):
        return packb(['pc', _prepare_response_message(response, self.send_body), [_prepare_request_message(link) for link in links]])

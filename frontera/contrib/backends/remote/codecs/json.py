...
def _convert_and_save_type(obj):
    ...
    if isinstance(obj, dict):
        return "dict", [
            (_convert_and_save_type(k), _convert_and_save_type(v))
...
def _convert_from_saved_type(obj):
    ...
    if obj_type == "dict":
        return {
            _convert_from_saved_type(k): _convert_from_saved_type(v)
...
class Encoder(BaseEncoder, CrawlFrontierJSONEncoder):
    ...
    def encode_new_job_id(self, job_id):
        return self.encode({"type": "new_job_id", "job_id": int(job_id)})

    def encode_offset(self, partition_id, offset):
        return self.encode(
            {"type": "offset", "partition_id": int(partition_id), "offset": int(offset)}
        )
...
class Decoder(json.JSONDecoder, BaseDecoder):
    ...
    def _response_from_object(self, obj):
        url = obj["url"]
        request = self._request_model(url=url, meta=obj["meta"])
    ...
    def decode(self, message):
        ...
        if message["type"] == "update_score":
            return (
                "update_score",
                self._request_from_object(message["r"]),
                message["score"],
                message["schedule"],
            )
